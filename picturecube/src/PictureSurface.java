...
public class PictureSurface implements ISurface {
	...
	public PictureSurface(PApplet parent, GLTexture orginTexture,
			GLTexture oldTexture, GLTexture shownTexture, LayerBlend filter) {
	...
	// public TextureSurface(PApplet parent, GLTexture shownTexture, GLTexture
	// orginTexture,
	// LayerBlend filter) {
	// this.shownTexture = shownTexture;
	// this.orginTexture = orginTexture;
	...
	public void draw() {
		if (oldTexture != null && ss != null) {
			...
			// drawCounterSteps+" scheise modulo: "+
			// drawCounter % drawCounterSteps);
			// if (drawCounter % drawCounterSteps == 0) {
			// System.out.println("its fading!!");

			if (picFullyShown) {
				long timeLeft = PictureChooser.PIC_SHOW_TIME
						- (timeWhenFadingEnded - timeWhenFadingStarted);
				...
				if (slideY >= 0.0 && slideY <= 1.0) {
					...
					float logisticSlide = (float) 1.02
							/ (1 + (float) Math.pow(Math.E,
									(1.02 * (-10) * (slideY - 0.5))));
					if (logisticSlide > 1.0)
						logisticSlide = 1;
					...
					// if(fadeCounter > 0) syOld = (int)((oldTexture.height -
					// heightOld) * logisticSlide);
					//
					...
					// if(fadeCounter > 0) sxOld = (int)((oldTexture.width -
					// widthOld) * logisticSlide);
					//
					...
					if (aspectRatioSS > aspectRatioOrgin) {
						...
						crop.setCrop(0, (orginTexture.height - heightOrgin)
								- syOrgin, orginTexture.width, heightOrgin);
						...
						crop.setCrop(sxOrgin, 0, widthOrgin,
								orginTexture.height);
						orginTexture.filter(crop, nextCropped);
					}
				}
		...
		if (fadeCounter < drawCounterSteps * fadeSteps) {
			...
			if (picFullyShown == false)
				timeWhenFadingEnded = System.currentTimeMillis();
			picFullyShown = true;
		}
	}
